...
{
    $sql = "SELECT p.*, c.categories AS category_name FROM product AS p";
    $sql .= " INNER JOIN categories AS c ON p.categories_id = c.id";
    ...
    if ($sub_categories != '') {
        $sql .= " AND p.sub_categories_id = $sub_categories";
    }
    if ($is_best_seller != '') {
        $sql .= " AND p.best_seller = 1";
    }
    if ($search_str != '') {
        $sql .= " AND (p.name like '%$search_str%' or p.description like '%$search_str%')";
    }
    if ($sort_order != '') {
        $sql .= $sort_order;
    } else {
...
function wishlist_add($con, $uid, $pid, $date_time)
{
    mysqli_query($con, "insert into wishlist(user_id,product_id,added_on) values('$uid','$pid','$date_time')");
}
...
function sentInvoice($con,$order_id){
    $res = mysqli_query($con, "SELECT DISTINCT od.id, od.*, p.name,p.image FROM order_detail as od INNER JOIN `order` as o ON od.order_id=o.id INNER JOIN product as p ON od.product_id=p.id WHERE od.order_id='$order_id'");

    $user_order = mysqli_fetch_assoc(mysqli_query($con, "SELECT o.*, u.name,u.email FROM `order` AS o INNER JOIN users AS u ON o.user_id=u.id WHERE o.id='$order_id'"));
